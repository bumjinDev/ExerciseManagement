# clocklog_to_xlsx.py — 클럭 CSV(typeperf)와 프로브 콘솔 로그를 묶어 클럭 정리 엑셀을 만든다.
#
# 입력 두 개:
...
#   회차별 측정값       : 측정 5회 각각의 원값(elapsed·procCPU·mainCPU 등). IMPLOG run 레코드가 있을 때만 만든다.
    ...
    for line in lines:
        ...
        if line.startswith('IMPLOG,leaf,'):
            ...
            leaves.append(dict(thread=p[2], par=int(p[3]), split=int(p[4]), run=int(p[5]),
                               scan=int(p[6]), loop_ns=int(p[8]), epoch_ms=int(p[13])))
            continue
...
def build_workbook(procs, clock, phases, thread_rows, pool_rows, runs, base_ghz, out_path):
    wb = Workbook()
    ...
    for name, st, en, thr, is_whole in phases:
        ...
        ws2.cell(row=r, column=7,
                 value=f'=AVERAGEIFS({rng_v},{rng_t},">="&B{r},{rng_t},"<="&C{r})').number_format = '0.0'
        ws2.cell(row=r, column=8, value=f"=G{r}*'클럭 정리'!$B$2/100")
    ...
    for tr in thread_rows:
        ...
        ws4.cell(row=r, column=10,
                 value=f'=AVERAGEIFS({rng_v},{rng_t},">="&E{r},{rng_t},"<="&F{r})').number_format = '0.0'
        c = ws4.cell(row=r, column=11, value=f"=J{r}*'클럭 정리'!$B$2/100")
    ...
    if runs:
        ...
        for rec in runs:
            ...
            rr += 1
        for j, w in enumerate([7, 8, 7, 6, 12, 12, 11, 11, 13, 12, 12, 8, 11], start=1):
...
def main():
    ...
    build_workbook(procs, clock, phases, thread_rows, pool_rows, runs, args.base, args.output)
    if not runs:
